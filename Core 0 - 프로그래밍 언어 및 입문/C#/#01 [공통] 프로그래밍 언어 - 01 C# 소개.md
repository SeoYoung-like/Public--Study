...
* 윈도우즈 개발을 위해서는 .NET 프레임워크가 필요 했으며, 동시에 C#을 사용 할 수 밖에 없게 만들었다.
...
[참고] https://rito15.github.io/posts/cs-dotnet-compile/
...
C#은 .NET Framework에 최적화 된 언어이다. MS .NET 프로그램이 동작할 수 있는 .NET 플랫폼에서 가장 직접적으로 반영하고, .NET 플랫폼에 강하게 의존하는 프로그래밍 언어이다. ( C# 외에도 베이직, 포트란, 파이썬, C++로 사용 가능하긴 하지만 CLI를 지켜야 하기 때문에 생산성 면에서 제약이 있다. )
...
2. **Multi Language** 
   ...
   * VB, C/C++, PHP 등도 쓸 수 있게 해주었다. 
     ...
     >
     > * CLR ( Common Language Runtime ) - 닷넷 플랫폼에 맞춰서 관리 되는 특징을 가지게 된다.
     >
     >   (ex) Managed C++, VB.NET - CLI를 맞추다 보니 거의 다른 언어가 되어 버렸다.

   * <u>[핵심] C#은 닷넷을 위해 태어난 언어이기 때문에 CLI를 따로 맞출 필요가 없다.</u> 
     
     * 이런 상황이라 윈도우 애플리케이션 개발자는 자연스럽게 C#을 쓸 수 밖에 없게 된다. ( 정유율 상승 )
